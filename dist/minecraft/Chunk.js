...
export class Chunk {
    ...
    generateCubes() {
        ...
        let heightVal = 200.0;
        let octave2 = this.upsampleHelper(this.getExpandedMatrix(2, heightVal), 2);
        let octave4 = this.upsampleHelper(this.getExpandedMatrix(4, heightVal), 4);
        let octave8 = this.upsampleHelper(this.getExpandedMatrix(8, heightVal), 8);
        let octave16 = this.upsampleHelper(this.getExpandedMatrix(16, heightVal), 16);
        let texture1 = this.upsampleHelper(this.getExpandedMatrix(2, 3.0), 2);
        let texture2 = this.upsampleHelper(this.getExpandedMatrix(4, 3.0), 2);
        let texture3 = this.upsampleHelper(this.getExpandedMatrix(8, 3.0), 2);
        let heights = new Array(octave2.length);
        this.heights = new Array(64);
        let types = new Array(64);
        let total = 4 * this.cubes;
        ...
        for (let i = 0; i < 64; i += scale) {
            for (let j = 0; j < 64; j += scale) {
                let height = Math.floor(this.heights[i][j]);
                let val = 3;
    ...
    upsample(matrix, length) {
        let newLength = (length * 2) + 2;
        let newMatrix = Array.from(Array(newLength), _ => Array(newLength).fill(0));
    ...
    checkCollision(position, walkDir) {
        let y = Math.floor(position.y - 2);
    ...
    destroyBlock(ray, origin) {
        ...
        for (let i = 0; i < this.numCubes(); i++) {
            ...
            if (intersection.x < minVec.x || intersection.x > maxVec.x ||
