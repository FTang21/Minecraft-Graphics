...
export class GUI {
    ...
    onKeydown(key) {
        switch (key.code) {
            ...
            case "KeyT": {
                this.animation.speedTime();
                break;
            }
            case "KeyM": {
                let cursorX = this.width / 2;
                let cursorY = this.height / 2;
                let ndcX = (2.0 * cursorX) / this.width - 1.0;
                let ndcY = 1.0 - (2.0 * cursorY) / this.height;
                let clip = new Vec4([ndcX, ndcY, -1.0, 1.0]);
                let inverseProj = new Mat4();
                this.projMatrix().inverse(inverseProj);
                let inverseView = new Mat4();
                this.viewMatrix().inverse(inverseView);
                let rayEye = inverseProj.multiplyVec4(clip);
                rayEye = new Vec4([rayEye.x / rayEye.w, rayEye.y / rayEye.w, rayEye.z / rayEye.w, 0.0]);
                let rayWorld = new Vec3(inverseView.multiplyVec4(rayEye).xyz).normalize();
                this.animation.destroy(rayWorld, this.camera.pos());
                break;
            }
            case "Space": {
                this.animation.jump();
