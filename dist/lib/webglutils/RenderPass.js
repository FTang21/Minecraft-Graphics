...
export class RenderPass {
    ...
    updateUniform(key, newLightPos) {
        const gl = this.ctx;
        gl.useProgram(this.shaderProgram);
        this.addUniform(key, (gl, loc) => {
            gl.uniform4fv(loc, newLightPos.xyzw);
        });
        let uniform = this.uniforms.get(key);
        if (uniform) {
            uniform.location = gl.getUniformLocation(this.shaderProgram, key);
        }
    }
